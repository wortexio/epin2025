...
<button onclick="window.location='yuklemelerim';" class="btn btn-dark user-panel-menu-button active"><i style="margin-top:8px; font-size:30px;" class="fad fa-credit-card"></i>Yüklemelerim</button>
...
<?php $shopierOrder  = $vt->query("select * from shopierOrder where user_id='$alan' Order By id DESC LIMIT 10")->fetchAll(PDO::FETCH_ASSOC); foreach ($shopierOrder as $shopierOrder) {  ?>
<span type="text" disabled style="background-color:#171a21; color:white; padding:8px; font-size:12px;">Yüklenen <b><?php echo $shopierOrder["amount"];?> TRY</b> 	
  <?php if ($shopierOrder["status"]==1) {echo ' <span style="color:#18dc92;">YÜKLENDİ</span>';}

 elseif ($shopierOrder["status"]==0) {echo ' <span style="color:red;">BAŞARISIZ</span>';} ?></span>
<?php } ?>
